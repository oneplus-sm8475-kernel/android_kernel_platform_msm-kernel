...
struct msm_geni_serial_port {
	...
	ktime_t wakeup_edge_time;
	struct completion	host_vote_on;
};
...
static void msm_geni_wakeup_work(struct work_struct *work)
{
	...
	if (atomic_read(&port->check_wakeup_byte)) {
		reinit_completion(&port->host_vote_on);
		...
		ret = wait_for_completion_timeout(&port->host_vote_on,
				msecs_to_jiffies(WAKEBYTE_TIMEOUT_MSEC)); ////Wait for 2s to check if we can receive wakeup byte.

        if(!ret)  //ret = 0 mean timeout
		{
			UART_LOG_DBG(port->ipc_log_rx, uport->dev,
					  "%s:wakeup byte not found\n", __func__);
		}
		else
		{
			UART_LOG_DBG(port->ipc_log_rx, uport->dev,
					  "%s:wakeup byte received\n", __func__);
		}
		msm_geni_serial_power_off(uport);
	}
}
...
static irqreturn_t msm_geni_wakeup_isr(int isr, void *dev)
{
	...
	UART_LOG_DBG(port->ipc_log_rx, uport->dev, "%s time=%lldmsec\n",
			 __func__, ktime_to_ms(cur_time));
	spin_lock_irqsave(&uport->lock, flags);
	if (atomic_read(&port->check_wakeup_byte)) {
		spin_unlock_irqrestore(&uport->lock, flags);
		return IRQ_HANDLED;
	}
	port->wakeup_edge_time = cur_time;
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
    ...
    init_completion(&dev_port->host_vote_on);
